...
void setup() {
  ...
  Serial0.setTxBufferSize(16384);
  Serial0.setRxBufferSize(16384);

  //Start USB Serial for debug messages
  Serial.begin(debug_baud);

  //Start UART Serial
  Serial0.setTimeout(3); //set timeout to 3 milliseconds, will transmit after timeout
  Serial0.begin(uart_baud);
...
void loop() {
  if(Serial0.available() && espnow_send_ready){
    uint bytes_received = Serial0.readBytes(&wifi_tx_buffer[1], 1469);
  ...
  while(true) {
    ...
    Serial0.write(temp_buf, temp_length);
